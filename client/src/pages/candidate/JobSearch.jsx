...
export default function JobSearch() {
  ...
  const filtered = jobs
    ...
    .sort((a, b) => {
      if (a.job.promoted !== b.job.promoted) return a.job.promoted ? -1 : 1;
  ...
  useEffect(() => {
    goToPage(1);
  }, [search, workMode, type, location, minMatch, personaId]);
  ...
  return (
    <div>
      ...
      {/* sticky search */}
      <div className="neo-card sticky top-4 z-10 mb-6 overflow-hidden rounded-2xl p-4">
        ...
        {showFilters && (
          <div className="mt-4 flex flex-col gap-y-3 border-t border-slate-700/30 pt-4 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-6 sm:gap-y-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {[0, 60, 70, 80].map((v) => (
                <button
                  ...
                  className={`rounded-lg px-3 py-1 text-xs font-semibold transition ${minMatch === v ? "bg-amber-500/15 text-amber-300" : "neo-secondary"
                    }`}
                >
                  {v === 0 ? "Any" : `${v}%+`}
                </button>
            ...
            <span className="neo-muted self-end text-xs sm:ml-auto">{filtered.length} jobs found</span>
          </div>
        )}
      </div>
      ...
      <div className="space-y-4">
        {paged.map(({ job, match }) => {
          ...
          return (
            <div key={job.id} className="neo-card relative rounded-2xl p-4 sm:p-6">
              {job.promoted && (
                <span className="absolute right-0 top-0 rounded-bl-xl rounded-tr-2xl bg-amber-500/20 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-amber-300">
                  Promoted
                </span>
              )}
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {companyRecord?.verified && (
                        <BadgeCheck size={14} className="shrink-0 text-blue-400" title="Verified employer" aria-label="Verified employer" />
                      )}
                    </div>
                    {companyRecord && (
                      <Link
                ...
                <div className="text-right shrink-0">
                  <p className={`text-2xl font-bold sm:text-3xl ${matchColor(match.score)}`}>{match.score}%</p>
...
function FilterGroup({ label, options, value, onChange }) {
  return (
    <div className="flex flex-wrap items-center gap-2">
      <span className="neo-muted whitespace-nowrap text-xs font-medium">{label}:</span>
      {options.map((o) => (
        <button
          key={o}
          onClick={() => onChange(o)}
          className={`whitespace-nowrap rounded-lg px-3 py-1 text-xs font-semibold transition ${value === o ? "bg-amber-500/15 text-amber-300" : "neo-secondary"
            }`}
        >
          {o}
        </button>
